#!/usr/bin/env python3
"""
FPGA UDP camera viewer for the current actual-image protocol.

Protocol:
  - 480x270 RGB565 image.
  - 270 UDP packets per frame, one image row per packet.
  - Packet 1 payload: pkt_num(4) + frame_header(4) + resolution(4) + row0(960) = 972 bytes.
  - Packet 2..270 payload: pkt_num(4) + row(960) = 964 bytes.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='FPGA UDP actual-image viewer, 480x270 RGB565.'
    )
