    ...
    if(!rst_n) begin
        ...
        case(next_state)
            ...
            st_tx_data : begin
					...
					if(data_cnt < tx_data_num) begin
						// 正常发送有效数据
						data_cnt <= data_cnt + 16'd1;
						case(tx_bit_sel)
							2'd0: gmii_txd <= tx_data[23:16];
							2'd1: gmii_txd <= tx_data[31:24];
							2'd2: begin
								gmii_txd <= tx_data[7:0];
								if(data_cnt != tx_data_num - 16'd2)
									tx_req <= 1'b1;
							end
							2'd3: gmii_txd <= tx_data[15:8];
						endcase
						// 若当前发送的是最后一个有效字节，且不需要填充，则提前置 skip_en
						if((data_cnt == tx_data_num - 16'd1) && (real_tx_data_num == tx_data_num)) begin
							skip_en <= 1'b1;
							data_cnt <= 16'd0;          // 复位，为下次准备
							tx_bit_sel <= 2'd0;
							// 注意：此时仍处于正常发送分支，gmii_txd 已赋有效值，不会被覆盖
						end
					end else begin
						// 填充分支（仅当 real_tx_data_num > tx_data_num 时才会进入此分支）
						gmii_txd <= 8'd0;
