module eth_img_pkt(
    ...
    //图像相关信号
    input                 cam_pclk       /* synthesis PAP_MARK_DEBUG="1" */,   //像素时钟
    input                 img_vsync      /* synthesis PAP_MARK_DEBUG="1" */,   //帧同步信号
    input                 img_data_en    /* synthesis PAP_MARK_DEBUG="1" */,   //数据有效使能信号
    input        [15:0]   img_data       ,   //有效数据 
    
    input                 transfer_flag  ,   //图像开始传输标志,1:开始传输 0:停止传输
    //以太网相关信号 
    input                 eth_tx_clk     ,   //以太网发送时钟
    input                 udp_tx_req     /* synthesis PAP_MARK_DEBUG="1" */,   //udp发送数据请求信号
...
parameter  CMOS_V_PIXEL = 16'd360;  //图像垂直方向分辨率
parameter  UDP_DATA_SIZE = 16'd1280; //UDP数据长度（不包含首部）
parameter  ETH_TRAN_DELAY = 16'd4375; //帧间隔延迟
//图像帧头,用于标志一帧数据的开始
...
reg f_vsync_txc_d1; */
//wire define                   
wire            pos_vsync       /* synthesis PAP_MARK_DEBUG="1" */;  //帧有效信号上升沿
wire            neg_vsync       /* synthesis PAP_MARK_DEBUG="1" */;  //帧有效信号下降沿
wire            neg_vsynt_txc   ;  //以太网时钟帧下降沿刹车信号的上升沿检测（伪帧下降沿检测）
wire            pos_de          ;  //数据有效信号上升沿
wire            pos_eth_delay   ;  //udp
wire   [15:0]    fifo_rdusedw    /* synthesis PAP_MARK_DEBUG="1" */;  //当前FIFO缓存的个数
/* wire 			pos_vsynt_txc;	 */	
...
always @(posedge cam_pclk or negedge rst_n) begin
    if(!rst_n) begin
		...
		if(!pos_reach)begin 
		if(pos_delay_cnt == 'd292500)begin 
			pos_delay_cnt <= 'd0;
			pos_reach <= 1'd1;
...
always @(posedge cam_pclk or negedge rst_n) begin
    if(!rst_n) begin
		...
		if(!neg_reach)begin 
		if(neg_delay_cnt == 'd281560)begin 
			neg_delay_cnt <= 'd0;
			neg_reach <= 1'd1;
...
//////////
reg cnt;//行de下降沿计数。上层文件裁剪导致一行de会有会变化两次
always @(posedge cam_pclk or negedge rst_n) begin
    if(!rst_n) begin
        cnt <= 1'b0;
    end
