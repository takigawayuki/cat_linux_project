"""
绿牌 pipeline：FPGA UDP → YOLO定位 → LPRNet识别
FPGA不做定位，RK端自己跑YOLO找车牌，再送LPRNet识别。
"""
import sys
import time
import numpy as np
...
from Drivers.udp_camera import UDPCamera
...
def draw_result(img_bgr, x1, y1, x2, y2, plate_text, det_score, lpr_conf):
    ...
    draw.text((x1, max(y1 - 22, 0)),
              f'{plate_text}  det={det_score:.2f} lpr={lpr_conf:.2f}',
...
def main():
    ...
    lpr.load_rknn(LPR_GREEN)
    lpr.init_runtime()
    print('done')

    co  = COCO_test_helper(enable_letter_box=True)

    win = 'Green Plate YOLO+LPR (FPGA UDP)'
    ...
    with UDPCamera() as cam:
        while True:
            frame, _, cap_fps = cam.read()   # ROI包忽略，YOLO自己定位
            ...
            if boxes is not None:
                ...
                for box, score in zip(real_boxes, scores):
                    ...
                    print(f'[GREEN] {plate}  det={score:.3f}  lpr={lpr_conf:.3f}')
                    vis = draw_result(vis, x1, y1, x2, y2, plate, score, lpr_conf)
